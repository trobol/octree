...
unsigned int VoxFile::DEFAULT_VOX_PALETTE[256] = {
...
void  VoxFile::readChunk_SIZE(FILE* file, int size) {
	mSize.x = readInt(file);
	mSize.z = readInt(file); //the z axis is up for .vox
...
void VoxFile::load(std::string path)
{
	...
	while (!feof(file)) {
		...
		case 0: break;
		case MAIN:
			throw "Found multiple MAINs";
			break;
		case SIZE:
			readChunk_SIZE(file, size);
			break;
		case XYZI:
			readChunk_XYZI(file, size);
			break;
		case RGBA:
			readChunk_RGBA(file, size);
			break;
		default:
			char* str = (char*)&type;
